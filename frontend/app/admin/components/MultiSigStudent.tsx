    ...
    return (
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
            <div className='flex flex-row items-end gap-6'>
                ...
                <div>
                    <input disabled={action == "Edit" ? true : false} onChange={handleFirstnameChange} value={firstname} type="text" className={`${action == "Edit" ? "cursor-not-allowed" : ""} h-10 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`} placeholder="Firstname" required />
                </div>
                <div>
                    <input disabled={action == "Edit" ? true : false} onChange={handleLastnameChange} value={lastName} type="text" className={`${action == "Edit" ? "cursor-not-allowed" : ""} h-10 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`} placeholder="Lastname" required />
                </div>
                <div>
                    <input disabled={action == "Edit" ? true : false} onChange={handleBirthdateChange} value={birthdate} type="date" className={`${action == "Edit" ? "cursor-not-allowed" : ""} appearance-none h-10 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`} placeholder="Birthdate" required />
                </div>
                <div>
